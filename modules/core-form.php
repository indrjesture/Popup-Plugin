											...
											if (array_key_exists($argument_matches[1][$k], $this->form_data)) {
												...
												if ($this->is_element_visible($argument_matches[1][$k])) {
													if (in_array($argument_matches[1][$k], $file_elements)) {
														...
														$replacement = 0;
														foreach ($this->form_data[$argument_matches[1][$k]] as $var_value) {
													...
													} else if (is_numeric($this->form_data[$argument_matches[1][$k]])) $replacement = $this->form_data[$argument_matches[1][$k]];
												}
			...
			foreach ($_filters as $filter) {
				switch($filter['type']) {
					case 'trim':
						...
						break;
					case 'alpha':
						if ($filter['properties']['whitespace-allowed'] == 'on') $value = preg_replace('/[^a-zA-Z\s]/', '', $value);
						else $value = preg_replace('/[^a-zA-Z]/', '', $value);
						break;
					case 'alphanumeric':
						if ($filter['properties']['whitespace-allowed'] == 'on') $value = preg_replace('/[^a-zA-Z0-9\s]/', '', $value);
						else $value = preg_replace('/[^a-zA-Z0-9]/', '', $value);
						break;
					case 'digits':
		...
		foreach ($values as $key => $value) {
			foreach ($_validators as $validator) {
				...
				switch($validator['type']) {
					case 'alpha':
						if ($validator['properties']['whitespace-allowed'] == 'on') $match = !preg_match('/[^a-zA-Z\s]/', $value);
						else $match = !preg_match('/[^a-zA-Z]/', $value);
						break;
					case 'alphanumeric':
						if ($validator['properties']['whitespace-allowed'] == 'on') $match = !preg_match('/[^a-zA-Z0-9\s]/', $value);
						else $match = !preg_match('/[^a-zA-Z0-9]/', $value);
						break;
					case 'date':
