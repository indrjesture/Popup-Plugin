...
class lepopup_class_targeting {
	...
	function admin_page() {
		...
		$rows = $wpdb->get_results("SELECT t1.*, t2.name as form_name, t3.name as form_mobile_name, t4.name as campaign_name, t5.name as campaign_mobile_name FROM ".$wpdb->prefix."lepopup_targets t1 LEFT JOIN ".$wpdb->prefix."lepopup_forms t2 ON t2.slug = t1.item LEFT JOIN ".$wpdb->prefix."lepopup_forms t3 ON t3.slug = t1.item_mobile LEFT JOIN ".$wpdb->prefix."lepopup_campaigns t4 ON t4.slug = t1.item LEFT JOIN ".$wpdb->prefix."lepopup_campaigns t5 ON t5.slug = t1.item_mobile WHERE t1.deleted = '0' AND t1.active = '1' AND t1.event = '".esc_sql($event)."' AND t1.post_type IN ('".implode("','", $static_types)."','".implode("','", $post_types)."')".$language_filter." ORDER BY t1.priority ASC", ARRAY_A);
		echo '
<div class="wrap lepopup-admin">
	...
	<div class="lepopup-options lepopup-targets-page">
		...
		$rows = $wpdb->get_results("SELECT t1.*, t2.name as form_name, t3.name as form_mobile_name, t4.name as campaign_name, t5.name as campaign_mobile_name FROM ".$wpdb->prefix."lepopup_targets t1 LEFT JOIN ".$wpdb->prefix."lepopup_forms t2 ON t2.slug = t1.item LEFT JOIN ".$wpdb->prefix."lepopup_forms t3 ON t3.slug = t1.item_mobile LEFT JOIN ".$wpdb->prefix."lepopup_campaigns t4 ON t4.slug = t1.item LEFT JOIN ".$wpdb->prefix."lepopup_campaigns t5 ON t5.slug = t1.item_mobile WHERE t1.deleted = '0' AND t1.active = '0' AND t1.event = '".esc_sql($event)."' AND t1.post_type IN ('".implode("','", $static_types)."','".implode("','", $post_types)."')".$language_filter." ORDER BY t1.created DESC", ARRAY_A);
		echo '
	<div class="lepopup-options lepopup-targets-page">
...
<div class="lepopup-dialog" id="lepopup-dialog">
	...
	function admin_target_properties() {
		...
		if (current_user_can('manage_options') || $lepopup->demo_mode) {
			...
			$target_details = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix."lepopup_targets WHERE deleted = '0' AND id = '".intval($_REQUEST['target-id'])."'", ARRAY_A);
			if ($target_details) {
			...
			$all_events = array_merge($this->events, $this->inline_events);
			if (array_key_exists('target-event', $_REQUEST) && array_key_exists($_REQUEST['target-event'], $all_events)) $event = $_REQUEST['target-event'];
			else $event = 'onload';
			$html = '
	...
	function admin_target_taxonomies() {
		...
		if (current_user_can('manage_options') || $lepopup->demo_mode) {
			...
			$all_events = array_merge($lepopup->events, $this->inline_events);
	...
	function get_taxonomies_html($_event, $_post_type, $_target_options) {
		...
		if ($_post_type == 'sitewide' || $_post_type == 'homepage') return '';
	...
	function admin_save() {
		...
		if (current_user_can('manage_options') || $lepopup->demo_mode) {
			...
			if ($target_details['post_type'] != 'sitewide' && $target_details['post_type'] != 'homepage') {
			...
			$target_details = $wpdb->get_row("SELECT t1.*, t2.name as form_name, t3.name as form_mobile_name, t4.name as campaign_name, t5.name as campaign_mobile_name FROM ".$wpdb->prefix."lepopup_targets t1 LEFT JOIN ".$wpdb->prefix."lepopup_forms t2 ON t2.slug = t1.item LEFT JOIN ".$wpdb->prefix."lepopup_forms t3 ON t3.slug = t1.item_mobile LEFT JOIN ".$wpdb->prefix."lepopup_campaigns t4 ON t4.slug = t1.item LEFT JOIN ".$wpdb->prefix."lepopup_campaigns t5 ON t5.slug = t1.item_mobile WHERE t1.id = '".esc_sql($target_id)."'", ARRAY_A);			
	...
	function get_events_data($_raw_post_id) {
		global $lepopup;
		$post_id = 0;
		...
		$targets = $this->get_targets($this->events, $post_id);
	...
	function get_targets($_events, $_post_id) {
