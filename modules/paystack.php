...
class lepopup_paystack_class {
	...
	function __construct() {
		if (is_admin()) {
			...
			add_action('wp_ajax_lepopup-paystack-settings-html', array(&$this, "admin_settings_html"));
			add_filter('lepopup_payment_gateways_transaction_html_paystack', array(&$this, 'admin_details'), 10, 3);
		}
		add_filter('lepopup_payment_gateways_do_paystack', array(&$this, 'front_submit'), 10, 2);
		add_action("init", array(&$this, "front_init"));
	}
	
	function providers($_providers) {
		if (!array_key_exists("paystack", $_providers)) $_providers["paystack"] = esc_html__('Paystack', 'lepopup');
		return $_providers;
	}
		...
		if (current_user_can('manage_options')) {
			...
			<div class="lepopup-properties-item">
				...
				<div class="lepopup-properties-content">
					...
					<label class="lepopup-integrations-description">'.sprintf(esc_html__('Find it on %sAPI Keys%s page.', 'lepopup'), '<a href="https://dashboard.paystack.com/#/settings/developer" target="_blank">', '</a>').'</label>
				</div>
			</div>
			<div class="lepopup-properties-item">
				<div class="lepopup-properties-label">
			...
			<div class="lepopup-properties-item">
				...
				<div class="lepopup-properties-tooltip">
					<i class="fas fa-question-circle lepopup-tooltip-anchor"></i>
					<div class="lepopup-tooltip-content">'.esc_html__('Please enter the email address where an HTML receipt will be sent to. Make sure this is a valid email address.', 'lepopup').'</div>
				</div>
				<div class="lepopup-properties-content">
					<div class="lepopup-input-shortcode-selector">
						<input type="text" name="email" value="'.(empty($data['email']) ? '' : esc_html($data['email'])).'" />
						<div class="lepopup-shortcode-selector" onmouseover="lepopup_shortcode_selector_set(this)";><span><i class="fas fa-code"></i></span></div>
					</div>
				</div>
			</div>
	...
	function front_submit($_result, $_data) {
		...
		if (empty($data['secret-key']) || empty($data['secret-key'])) return $_result;
		...
		$post_data = array(
			'amount' => intval(100*$data['amount']),
			'currency' => $data['currency'],
			'email' => $data['email'],
			'reference' => $data["record-id"].'-'.time(),
			'metadata' => array(
				'custom_fields' => array(
					array(
						'display_name' => 'Item',
						'variable_name' => 'item_name',
						'value' => (!empty($data['item-name']) ? $data['item-name'] : 'Fee')
					)
				)
			),
			'callback_url' => (empty($data['redirect-url']) ? $_SERVER["HTTP_REFERER"] : $data['redirect-url'])
		);
		$payment = $this->connect($data['secret-key'], 'transaction/initialize', $post_data);
		if (is_array($payment) && array_key_exists('status', $payment)) {
			if ($payment['status'] === true) {
				$html = '
	<form action="'.$payment['data']['authorization_url'].'" method="get" target="_top" style="display: none !important;">
		<input type="submit" class="lepopup-pay" value="Submit">
				...
				$result = array('status' => 'ERROR', 'message' => rtrim($payment['message'], '.').'.');
			}
		} else {
			$result = array('status' => 'ERROR', 'message' => esc_html__('Can not connect to Paystack server.', 'lepopup'));
		}
		return $result;
	}
	...
	function front_init() {
		global $wpdb, $lepopup;
		$form_object = null;
		if (array_key_exists('lepopup-ipn', $_REQUEST) && $_REQUEST['lepopup-ipn'] == 'paystack') {
			...
			if (empty($post_data) || !is_array($post_data) || !array_key_exists('event', $post_data) || $post_data['event'] != 'charge.success') {
				http_response_code(400);
				exit;
			}
			$transaction_details = $post_data;

			$item_id = $post_data['data']['reference'];
			if (($pos = strpos($item_id, "-")) !== false) $item_id = intval(substr($item_id, 0, $pos));
			$payment_status = $post_data['data']['status'];
			$transaction_type = $post_data['data']['channel'];
			$txn_id = $post_data['data']['id'];
			$payer_id = $post_data['data']['customer']['email'];
			$gross_total = $post_data['data']['amount'];
			$mc_currency = $post_data['data']['currency'];
			$payer_name = empty($post_data['data']['customer']['customer_code']) ? 'Paystack Payer' : $post_data['data']['customer']['customer_code'];
				...
				if (!$record_details || !is_array($record_details) || !array_key_exists('gateway_id', $record_details)) $payment_status = "Error: no record";
					...
					if (empty($form_object->id)) $payment_status = "Error: no form";
						...
						if (empty($payment_gateway) || !is_array($payment_gateway)) $payment_status = "Error: no payment gateway";
							...
							if ($signature != hash_hmac('sha512', $payload, $payment_gateway['data']['secret-key'])) $payment_status = "Error: invalid hash";
							else if (intval($gross_total) < intval(100*$record_details["amount"]) || $mc_currency != $record_details["currency"]) $payment_status = "Error: invalid amount";
						}
					}
				}
			}
			$sql = "INSERT INTO ".$wpdb->prefix."lepopup_transactions (record_id, provider, payer_name, payer_email, gross, currency, payment_status, transaction_type, txn_id, details, created, deleted ) VALUES (
				...
				'".esc_sql($payer_name)."',
				'".esc_sql($payer_id)."',
				'".esc_sql(floatval($gross_total/100))."',
				'".esc_sql($mc_currency)."',
			...
			if ($payment_status == "Success") {
				...
				$form_object->do_notifications("payment-success", array('payment-amount' => $gross_total/100, 'payment-currency' => $mc_currency, 'payment-status' => $payment_status));
				...
				if (!empty($form_object) && !empty($form_object->id)) {
					...
					$form_object->form_info = json_decode($record_details['info'], true);
					$form_object->record_id = $record_details['id'];
					$form_object->do_notifications("payment-fail", array('payment-amount' => $gross_total/100, 'payment-currency' => $mc_currency, 'payment-status' => $payment_status));
	...
	function connect($_secret_key, $_path, $_data = array(), $_method = '') {
		$headers = array(
		...
		try {
			$url = 'https://api.paystack.co/'.ltrim($_path, '/');
			$curl = curl_init($url);
...
$lepopup_paystack = new lepopup_paystack_class();
?>