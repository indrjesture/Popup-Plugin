<?php
/* Conversio integration for Green Forms */
if (!defined('UAP_CORE') && !defined('ABSPATH')) exit;
class lepopup_conversio_class {
	var $default_parameters = array(
		...
		"custom-names" => array(),
		"custom-values" => array(),
		"optintext" => "Subscribe to receive newsletters"
	);
	
	function __construct() {
		if (is_admin()) {
			add_filter('lepopup_providers', array(&$this, 'providers'), 10, 1);
			add_action('wp_ajax_lepopup-conversio-settings-html', array(&$this, "admin_settings_html"));
			add_action('wp_ajax_lepopup-conversio-list', array(&$this, "admin_lists"));
		}
		add_filter('lepopup_integrations_do_conversio', array(&$this, 'front_submit'), 10, 2);
	}
	
	function providers($_providers) {
		if (!array_key_exists("conversio", $_providers)) $_providers["conversio"] = esc_html__('Conversio', 'lepopup');
		return $_providers;
	}
	...
	function admin_settings_html() {
		...
		if (current_user_can('manage_options')) {
			...
			<div class="lepopup-properties-item">
				...
				<div class="lepopup-properties-tooltip">
					...
					<div class="lepopup-tooltip-content">'.esc_html__('Enter your Conversio API Key.', 'lepopup').'</div>
				</div>
				<div class="lepopup-properties-content">
					<input type="text" name="api-key" value="'.esc_html($data['api-key']).'" />
					<label class="lepopup-integrations-description">'.sprintf(esc_html__('Find your Conversio API Key on %sProfile%s page.', 'lepopup'), '<a href="https://app.conversio.com/profile" target="_blank">', '</a>').'</label>
				</div>
			</div>
			<div class="lepopup-properties-item">
				<div class="lepopup-properties-label">
			...
			<div class="lepopup-properties-item">
				...
				<div class="lepopup-properties-tooltip">
					<i class="fas fa-question-circle lepopup-tooltip-anchor"></i>
					<div class="lepopup-tooltip-content">'.esc_html__('Map form fields to Conversio fields.', 'lepopup').'</div>
				</div>
				<div class="lepopup-properties-content">
		...
		$html .= '
			...
			<div class="lepopup-properties-item">
				<div class="lepopup-properties-label">
					<label>'.esc_html__('Opt-In Text', 'lepopup').'</label>
				</div>
				<div class="lepopup-properties-tooltip">
					<i class="fas fa-question-circle lepopup-tooltip-anchor"></i>
					<div class="lepopup-tooltip-content">'.esc_html__('What opt-in text was shown to the subscriber. This is required for GDPR compliance.', 'lepopup').'</div>
				</div>
				<div class="lepopup-properties-content">
					<input type="text" name="optintext" value="'.esc_html($data['optintext']).'" />
				</div>
			</div>';
			$return_object = array();
	...
	function admin_lists() {
		...
		if (current_user_can('manage_options')) {
			...
			$result = $this->connect($deps['api-key'], 'customer-lists');
			if (is_array($result)) {
				if (array_key_exists('errors', $result)) {
					$return_object = array('status' => 'ERROR', 'message' => $result['errors'][0]);
					echo json_encode($return_object);
					exit;
				}
				if (sizeof($result) > 0) {
	...
	function front_submit($_result, $_data) {
		...
		$post_data = array(
			...
			'source' => $data['form-name'],
			'sourceType' => 'Green Forms',
			'sourceId' => $data['form-id'],
			'optInText' => $data['optintext']
		);
		if (!empty($data['custom-names'])) {
			$post_data['properties'] = array();
			foreach($data['custom-names'] as $key => $name) {
				$name = preg_replace('/[^a-zA-Z0-9-_]/', '', $name);
				if (!empty($name) && !empty($data['custom-values'][$key])) $post_data['properties'][$name] = $data['custom-values'][$key];
			}
		}
		$result = $this->connect($data['api-key'], 'customer-lists/'.urlencode($data['list-id']).'/subscriptions', $post_data, 'PUT');
		return $_result;
	}
...
$lepopup_conversio = new lepopup_conversio_class();
?>