...
class lepopup_mailwizz_class {
	...
	function admin_settings_html() {
		...
		if (current_user_can('manage_options')) {
			...
			<div class="lepopup-properties-item">
				...
				<div class="lepopup-properties-tooltip">
					<i class="fas fa-question-circle lepopup-tooltip-anchor"></i>
					<div class="lepopup-tooltip-content">'.esc_html__('Enter your MailWizz API Public Key. You can generate it in MailWizz customer area.', 'lepopup').'</div>
				</div>
				<div class="lepopup-properties-content">
			...
			<div class="lepopup-properties-item">
				...
				<div class="lepopup-properties-tooltip">
					<i class="fas fa-question-circle lepopup-tooltip-anchor"></i>
					<div class="lepopup-tooltip-content">'.esc_html__('Enter your MailWizz API Private Key. You can generate it in MailWizz customer area.', 'lepopup').'</div>
				</div>
				<div class="lepopup-properties-content">
			...
			<div class="lepopup-properties-item">
				...
				<div class="lepopup-properties-content">
					<div class="lepopup-properties-group lepopup-integrations-ajax-options">
						<input type="text" name="list" value="'.esc_html($data['list']).'" data-deps="api-url,public-key,private-key" readonly="readonly" />
			...
			if (!empty($data['api-url']) && !empty($data['public-key']) && !empty($data['private-key']) && !empty($data['list-id'])) {
				$fields_data = $this->get_fields_html($data['api-url'], $data['public-key'], $data['private-key'], $data['list-id'], $data['fields']);
				if ($fields_data['status'] == 'OK') $html .= $fields_data['html'];
			}
			$html .= '
					</div>
					<a class="lepopup-button lepopup-button-small" onclick="return lepopup_integrations_ajax_inline_html(this);" data-inline="fields" data-deps="api-url,public-key,private-key,list-id"><i class="fas fa-download"></i><label>'.esc_html__('Load Fields', 'lepopup').'</label></a>
				</div>
			</div>';
			$return_object = array();
	...
	function admin_lists() {
		...
		if (current_user_can('manage_options')) {
			...
			if (!is_array($deps) || !array_key_exists('api-url', $deps) || empty($deps['api-url']) || !array_key_exists('public-key', $deps) || empty($deps['public-key']) || !array_key_exists('private-key', $deps) || empty($deps['private-key'])) {
				$return_object = array('status' => 'ERROR', 'message' => esc_html__('Invalid API Credentials.', 'lepopup'));
				echo json_encode($return_object);
				exit;
			}

			$result = $this->connect($deps['api-url'], $deps['public-key'], $deps['private-key'], 'lists?page=1&per_page=9999');
	...
	function admin_fields_html() {
		...
		if (current_user_can('manage_options')) {
			...
			if (!is_array($deps) || !array_key_exists('api-url', $deps) || empty($deps['api-url']) || !array_key_exists('public-key', $deps) || empty($deps['public-key']) || !array_key_exists('private-key', $deps) || empty($deps['private-key']) || !array_key_exists('list-id', $deps) || empty($deps['list-id'])) {
				$return_object = array('status' => 'ERROR', 'message' => esc_html__('Invalid API Credentials or List ID.', 'lepopup'));
				echo json_encode($return_object);
				exit;
			}
			$return_object = $this->get_fields_html($deps['api-url'], $deps['public-key'], $deps['private-key'], $deps['list-id'], $this->default_parameters['fields']);
			echo json_encode($return_object);
		}
		exit;
	}
	...
	function get_fields_html($_api_url, $_public_key, $_private_key, $_list, $_fields) {
		global $wpdb, $lepopup;
		$result = $this->connect($_api_url, $_public_key, $_private_key, 'lists/'.$_list.'/fields?page=1&per_page=9999');
		if(!$result || !is_array($result)) {
	...
	function front_submit($_result, $_data) {
		...
		if (empty($data['api-url']) || empty($data['public-key']) || empty($data['private-key']) || empty($data['list-id'])) return $_result;
		...
		$result = $this->connect($data['api-url'], $data['public-key'], $data['private-key'], 'lists/'.$data['list-id'].'/subscribers/search-by-email?EMAIL='.urlencode($data['fields']['EMAIL']));
		...
		if ($result['status'] != 'success') {
			$result = $this->connect($data['api-url'], $data['public-key'], $data['private-key'], 'lists/'.$data['list-id'].'/subscribers', 'POST', $post_data);
		} else {
			$result = $this->connect($data['api-url'], $data['public-key'], $data['private-key'], 'lists/'.$data['list-id'].'/subscribers/'.$result['data']['subscriber_uid'], 'PUT', $post_data);
		}
		return $_result;
	}
	
	function connect($_api_url, $_public_key, $_private_key, $_path, $_method = 'GET', $_data = null) {
		try {
