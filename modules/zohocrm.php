		...
		if (current_user_can('manage_options')) {
			...
			if (array_key_exists("do", $_REQUEST)) {
				switch ($_REQUEST['do']) {
					...
					default:
						$content = esc_html__('Inavlid URL. Please try again.', 'lepopup').'<div><a class="button" href="'.admin_url('admin-ajax.php').'?action=lepopup-zohocrm-connect"><i class="fas fa-check"></i><label>'.esc_html__('Try again', 'lepopup').'</label></a></div>';
						break;
				}
				echo '
<!DOCTYPE html>
...
<head>
	...
	<link rel="stylesheet" media="all" href="'.$lepopup->plugins_url.'/css/font-awesome.min.css" />
