...
//			}
	...
	function check_for_plugin_update($checked_data) {
		global $wp_version, $lepopup;
		
		$file = basename(dirname(dirname(__FILE__))).'/green-popups.php';
		$purchase_code = preg_replace('/[^a-zA-Z0-9-]/', '', $lepopup->options['purchase-code']);
		...
		$raw_response = wp_remote_post(LEPOPUP_API_URL, $request_string);
	...
	function plugin_api_call($def, $action, $args) {
		...
		if (!isset($args->slug) || ($args->slug != 'green-popups'))
			return false;
		...
		$request = wp_remote_post(LEPOPUP_API_URL, $request_string);
		
		if (is_wp_error($request)) {
			$res = new WP_Error('plugins_api_failed', esc_html__('An Unexpected HTTP Error occurred during the API request.', 'lepopup').' <a href="#" onclick="document.location.reload(); return false;">'.esc_html__('Try again.', 'lepopup').'</a>', $request->get_error_message());
		} else {
			$res = unserialize($request['body']);
			if ($res === false) {
				$res = new WP_Error('plugins_api_failed', esc_html__('An unknown error occurred', 'lepopup'), $request['body']);
			}
		}
		return $res;
	}
