	...
	function _lepopup_rebuild_active_element() {
		...
		if (lepopup_element_properties_active) {
			...
			if (idx) {
				...
				var output = _lepopup_build_children(lepopup_form_page_active, idx);
...
function _lepopup_build_hidden_list(_parent) {
	...
	if (html != "") html = "<div class='lepopup-hidden-container'><label>Hidden fields:</label>"+html+"</div>";
	return html;
}
...
function lepopup_build() {
	...
	if (lepopup_form_options["progress-enable"] == "on") {
		if (lepopup_form_options["progress-type"] == 'progress-2') {
...
function lepopup_target_properties_open(_event, _target_id) {
	...
	var post_data = {"action" : "lepopup-target-properties", "target-event" : _event, "target-id" : lepopup_target_properties_active};
	jQuery.ajax({
		type	: "POST",
		url		: lepopup_ajax_handler, 
		data	: post_data,
